...
@section('content')
    ...
    <div class="modal fade" id="overrideKelasModal" tabindex="-1" aria-labelledby="overrideKelasModalLabel"
        aria-hidden="true">
        ...
        $(document).ready(function() {
            ...
            var table = $('#peserta-table').DataTable({
                ...
                columns: [{
                    ...
                    {
                        data: 'subkategori_lomba.kategori_lomba.nama',
                        name: 'subkategoriLomba.kategoriLomba.nama'
                    },
                    {
                        data: 'kelompok_usia.nama',
                        name: 'kelompokUsia.nama'
                    },
                    {
                        data: 'kelas_tanding',
                        name: 'kelasTanding.label_keterangan'
                    },
                    {
            ...
            $('#peserta-table').on('click', '.verify-btn', function() {
                ...
                if (confirm(
                        `Apakah Anda yakin ingin mengubah status verifikasi peserta menjadi ${statusText}?`
                        )) {
                    $.ajax({
                        url: verifyUrl,
                        type: 'PUT',
            ...
            $('#peserta-table').on('click', '.override-kelas-btn', function() {
                ...
                // Set form action
                var overrideUrl = `{{ route('admin.peserta.override-class', ':id') }}`.replace(':id',
                    pesertaId);
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if (Array.isArray(items)) {
                            items.forEach(function(kelas) {
                                var selected = kelas.id == kelasTandingId ? 'selected' :
                                    '';
                                options +=
                                    `<option value="${kelas.id}" ${selected}>${kelas.label_keterangan} (${kelas.berat_min}-${kelas.berat_max} kg)</option>`;
                            });
                        }
                    ...
                    error: function(xhr) {
                        console.error('Error fetching kelas tanding:', xhr);
